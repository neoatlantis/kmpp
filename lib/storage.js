...
module.exports.getStorage = function(username){
    return storages[username] || null;
};
    ...
    if(credential){
        ...
        try{
            if(undefined == storages[username]){
                // TODO allowd username
                if(/^[0-9a-z]+$/i.test(username)){
                ...
                authed = storages[username].verifyPassphrase(password);
            };
        } catch(e){
        };
    };
