...
module.exports = function(storage, passphrase){
    ...
    if(
        ...
        var protectKey = crypto.pbkdf2Sync(
            passphrase,
            salt,
            constants.ITERATION,
            constants.KEY_SIZE
    ...
    if(mainKeyEncrypted && 0 == mainKeyEncrypted.length % 2){
        try{
            ...
            var mainKey = buffer.Buffer.concat([p1, p2]);
        } catch(e){
        };
    };
    if(false === mainKey){
        ...
        var mainKey = crypto.randomBytes(constants.KEY_SIZE);
    ...
    function hashKey(k){
        ...
        return hmac(kBuf).toString('base64');
    };
    ...
    var reverseLookupTableKey = hashKey('reverseLookupTable');
